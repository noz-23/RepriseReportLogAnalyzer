...
internal sealed class ListAnalysisCheckOutIn : List<AnalysisCheckOutIn>, IAnalysisOutputFile
{
    ...
    public void Analysis(ConvertReportLog log_, IEnumerable<AnalysisStartShutdown> listStartShutdown_)
    {
        ...
        foreach (var startShutdown in listStartShutdown_)
        {
            ...
            // 重複のチェック
            _setDuplicationEx(listCheckOutIn);
    ...
    private void _setDuplication(ConcurrentDictionary<string, List<AnalysisCheckOutIn>> listOutIn_)
    {
        ...
        foreach (var key in listOutIn_.Keys)
        {
    ...
    private void _setDuplicationEx(SortedDictionary<string, List<AnalysisCheckOutIn>> listOutIn_)
    {
        int count = 0;
        int max = listOutIn_.Keys.Count;
        ProgressCount?.Invoke(0, max, _ANALYSIS + "Duplication");
